...
interface NewArrivalsSectionProps {
  newArrivals: { name: string; imageUrl?: string; price?: string }[]
  title?: string
  subtitle?: string
}

export function NewArrivalsSection({ newArrivals, title, subtitle }: NewArrivalsSectionProps) {
      ...
      <section className="bg-background py-16">
        <div className="mx-auto max-w-7xl px-4">
          <div className="mb-8 flex items-center justify-between">
            <div>
              <h2 className="text-3xl font-bold text-foreground">{title || "Nuevo en Stock"}</h2>
...
interface FeaturedProductsSectionProps {
  products: { name: string; imageUrl?: string; price?: string; priceBefore?: string; brand?: string }[]
  title?: string
}

export function FeaturedProductsSection({ products, title }: FeaturedProductsSectionProps) {
  ...
  return (
    <FadeUp>
      <section className="bg-surface-light py-16">
        <div className="mx-auto max-w-7xl px-4">
          <div className="mb-8 flex items-center justify-between">
            <h2 className="text-3xl font-bold text-foreground">{title || "Productos Destacados"}</h2>
...
export function FeaturesSection({ features, title }: FeaturesSectionProps) {
  if (features.length === 0) return null
  return (
    <FadeUp>
      <section className="bg-background py-16">
        <div className="mx-auto max-w-7xl px-4">
          <h2 className="mb-10 text-center text-3xl font-bold text-foreground">{title}</h2>
          <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            {features.map((f, i) => (
...
export function StoreLocatorSection({ storeLocator }: StoreLocatorSectionProps) {
  if (!storeLocator.title) return null
  return (
    <section className="bg-surface-light py-16">
...
export function CategoryGridSection({ categories, iconMap, title }: CategoryGridSectionProps) {
  if (categories.length === 0) return null
  const catSlug = (cat: string) => cat.toLowerCase().replace(/[^a-z]/g, "")
  return (
    <section className="bg-background py-16">
      <div className="mx-auto max-w-7xl px-4">
        <h2 className="mb-6 text-center text-3xl font-bold text-foreground">{title}</h2>
          ...
          {categories.map((cat: string, i: number) => {
            ...
            return (
              <Link key={i} href={"/tienda#" + slug}
                ...
                <Image src={iconMap[slug] || ""} alt={cat} width={64} height={64} className="h-16 w-16" />
                ...
                <div className="rounded-xl border border-border bg-white p-6 shadow-sm">
                  <div className="mb-3 flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <div className="flex gap-1 text-amber-400">{Array.from({ length: t.rating || 5 }).map((_, j) => <span key={j}>★</span>)}</div>
                    </div>
                  </div>
...
export function PaymentMethodsSection({ installmentsText }: PaymentMethodsSectionProps) {
  return (
    <section className="bg-primary/5 py-10">
      <div className="mx-auto max-w-7xl px-4 text-center">
        ...
        {installmentsText && (
          <p className="mt-3 text-xs text-muted-foreground">{installmentsText}</p>
        )}
      </div>
    </section>
  )
...
export function NewsletterSection({ title, description, placeholder, buttonText }: NewsletterSectionProps) {
  return (
    <section className="bg-primary py-16">
      <div className="mx-auto max-w-3xl px-4 text-center">
        <h2 className="mb-4 text-3xl font-bold text-primary-foreground">{title || "Recibí Novedades"}</h2>
        <p className="mb-6 text-primary-foreground/80">{description}</p>
        <form action="/api/subscribe" method="POST" className="mx-auto flex max-w-md gap-3">
          <input name="email" type="email" placeholder={placeholder || "tu@email.com"}
            className="flex-1 rounded-lg border border-white/30 bg-white/10 px-4 py-3 text-white placeholder-white/50 outline-none focus:border-white" required />
          <button type="submit"
            className="rounded-lg bg-accent px-6 py-3 font-semibold text-accent-foreground transition-all hover:bg-accent/90">
    ...
    <FadeUp>
      <section className="relative overflow-hidden py-16" style={{ background: "linear-gradient(135deg, var(--color-accent) 0%, var(--color-primary) 100%)" }}>
        <div className="relative z-10 mx-auto max-w-3xl px-4 text-center">
          <h2 className="mb-4 text-3xl font-bold text-white">{finalCta.title}</h2>
          <p className="mb-6 text-white/80">{finalCta.description}</p>
          <div className="flex flex-col items-center gap-4 sm:flex-row sm:justify-center">
            <a href={finalCta.buttonLink} className="inline-block rounded-lg bg-white px-8 py-4 font-semibold text-accent transition-all hover:scale-105">{finalCta.buttonText}</a>
            {finalCta.secondaryText && <Link href={finalCta.secondaryLink || "/tienda"} className="inline-block rounded-lg border-2 border-white px-8 py-4 font-semibold text-white transition-all hover:bg-white/20">{finalCta.secondaryText}</Link>}
          </div>
        </div>
      </section>
    </FadeUp>
