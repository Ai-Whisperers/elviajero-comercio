...
export function ProductCard({ product, onClick }: { product: Product; onClick?: (p: Product) => void }) {
  ...
  const waUrl = getProductWhatsappUrl(
    product.name,
    product.price,
  ...
  return (
    <div
      className="group relative flex flex-col overflow-hidden rounded-lg border border-border bg-surface shadow-sm transition-all duration-200 hover:shadow-md"
    ...
    >
      ...
      <div className="relative">
        ...
        >
          ...
          {/* Sale badge */}
          {isOnSale && (
            <span className="absolute left-2 top-2 rounded-full bg-accent px-2 py-0.5 text-[10px] font-bold text-accent-foreground shadow-sm z-10">
              {LABELS.sale}
            </span>
          )}
          ...
          <button
            ...
            className={`absolute right-2 top-2 z-20 flex h-7 w-7 items-center justify-center rounded-full shadow-sm transition-all duration-200 ${
          ...
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="14" height="14" viewBox="0 0 24 24" fill={isWished(product.name) ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2">
      ...
      <div className="flex flex-1 flex-col p-2.5">
        <Link href={productHref} onClick={() => onClick?.(product)} className="block min-h-0">
          <h3 className="text-xs font-semibold text-foreground line-clamp-2 transition-colors group-hover:text-primary leading-snug">
            {product.name}
          </h3>
          {product.specs && (
            <p className="mt-0.5 text-[10px] text-muted-foreground/50 line-clamp-1">{product.specs}</p>
          )}
        </Link>
        ...
        <div className="mt-auto flex items-baseline gap-1.5 pt-1">
          ...
          {isOnSale && product.priceBefore && (
            <p className="text-[10px] text-muted-foreground line-through">{product.priceBefore}</p>
          )}
        </div>
        ...
        <div className="mt-2 grid grid-cols-2 gap-1.5">
          <button
            onClick={handleAdd}
            disabled={!hasStock}
            className="flex h-8 items-center justify-center gap-1 rounded-md bg-primary px-2 text-[10px] font-semibold text-primary-foreground transition-all hover:bg-primary/90 active:scale-[0.97] disabled:cursor-not-allowed disabled:opacity-50"
            aria-label={`Agregar ${product.name} al carrito`}
          >
            <svg xmlns="http://www.w3.org/2000/svg" width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" className="shrink-0">
          ...
          <a
            ...
            className="flex h-8 items-center justify-center gap-1 rounded-md bg-green-600 px-2 text-[10px] font-semibold text-white transition-all hover:bg-green-700 active:scale-[0.97]"
