        ...
        <div className="rounded-xl border border-zinc-800/60 bg-zinc-900/50 overflow-hidden">
          <div className="p-4 border-b border-zinc-800/60">
            <input type="text" value={search} onChange={e => setSearch(e.target.value)}
              ...
              className="w-full rounded-lg border border-zinc-700/60 bg-zinc-800 px-3 py-2 text-sm text-white outline-none focus:border-emerald-500/50 placeholder-zinc-500" />
          </div>
            ...
            {loading ? (
              ...
              filteredProfiles.map(p => (
                <button key={p.id} onClick={() => setSelected(p.id)}
                  className={`w-full text-left p-4 transition-colors hover:bg-zinc-800/50 ${selected === p.id ? "bg-emerald-600/10 border-l-2 border-green-500" : ""}`}>
                  <p className="font-medium text-white text-sm">{p.name || "Sin nombre"}</p>
                    <p className="text-sm text-zinc-500">{p.phone || "Sin contacto"}</p>
                  {p.phone && <p className="text-xs text-zinc-600 mt-0.5">{p.phone}</p>}
                </button>
              ))
            )}
          </div>
        ...
        <div>
          {!selected ? (
            <div className="flex flex-col items-center justify-center py-24 text-zinc-500 rounded-xl border border-zinc-800/60 bg-zinc-900/50">
              ...
              <p className="text-base font-medium text-zinc-400">Seleccioná un cliente</p>
              <p className="text-sm text-zinc-600 mt-1">para ver su historial de pedidos</p>
            </div>
          ) : (
            <>
              {/* Customer header */}
              <div className="mb-6 rounded-xl border border-zinc-800/60 bg-zinc-900/50 p-6">
                <div className="flex items-start justify-between mb-4">
                  ...
                  <div className="text-right">
                    <p className="text-2xl font-bold text-emerald-400">Gs. {totalSpent.toLocaleString("es-PY")}</p>
              ...
              {/* Orders list */}
              <h3 className="text-sm font-semibold text-zinc-400 mb-3 uppercase tracking-wider">Pedidos</h3>
              {orders.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-zinc-500 rounded-xl border border-zinc-800/60 bg-zinc-900/50">
                  <svg className="w-10 h-10 mb-3 text-zinc-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2" /></svg>
                ...
                <div className="space-y-2">
                  {orders.map(o => (
                    <Link key={o.id} href={"/admin/pedidos/detalle?id=" + o.id}
                      className="block rounded-xl border border-zinc-800/60 bg-zinc-900/50 p-4 hover:border-zinc-700/60 transition-all">
                      <div className="flex items-center justify-between">
                        ...
                        <div className="text-right">
                          ...
                          <span className={`inline-block rounded-full px-2.5 py-0.5 text-[10px] font-semibold border ${statusColors[o.status] || "bg-zinc-800 text-zinc-400 border-zinc-700/60"}`}>
                            {o.status}
                          </span>
                        </div>
                      </div>
