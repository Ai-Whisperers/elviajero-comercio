      ...
      <div className="flex items-end gap-1.5 h-32">
        {data.map((d, i) => (
          <div key={i} className="flex-1 flex flex-col items-center gap-1">
            <div className="w-full rounded-t bg-green-500/30 hover:bg-green-500/50 transition-all min-h-[4px]" style={{ height: `${(d.value / max) * 100}%` }} />
    ...
    <>
      ...
      <div className="mb-6 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-gray-500 mt-0.5">{orders.length} pedidos en total · {filtered.length} en el período</p>
        </div>
        <div className="flex items-center gap-2">
          <input type="date" value={dateFrom} onChange={e => setDateFrom(e.target.value)}
            className="rounded-lg bg-gray-800 border border-gray-700 px-3 py-2 text-xs text-white outline-none focus:border-green-500" />
      ...
      {loading ? (
          ...
          <div className="grid grid-cols-4 gap-4">
            {Array.from({ length: 4 }).map((_, i) => (
              <div key={i} className="rounded-xl border border-gray-800 bg-gray-900 p-5">
                ...
                <div className="h-6 w-24 rounded bg-gray-800 mb-2" />
                <div className="h-3 w-20 rounded bg-gray-800" />
              </div>
            ))}
          </div>
          <div className="h-32 rounded-xl bg-gray-900 border border-gray-800" />
        </div>
      ) : orders.length === 0 ? (
        /* Empty state */
        <div className="flex flex-col items-center justify-center py-24 text-gray-500 rounded-xl border border-gray-800 bg-gray-900">
          ...
          <p className="text-base font-medium text-gray-400 mb-1">No hay datos de ventas</p>
          <p className="text-sm text-gray-600">Los reportes aparecerán cuando haya pedidos</p>
        </div>
      ) : (
        <>
          {/* Stats cards */}
          ...
          <div className="mb-4 flex items-center justify-between">
            ...
            <button onClick={() => exportOrdersCSV(filtered)}
              className="rounded-lg border border-gray-700 px-4 py-2 text-xs text-gray-400 hover:text-white hover:border-gray-600 transition-all flex items-center gap-1.5">
          ...
          {topProducts.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-12 text-gray-500 rounded-xl border border-gray-800">
              <svg className="w-10 h-10 mb-3 text-gray-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1} d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 4v10M4 7v10l8 4" /></svg>
            ...
            <div className="overflow-x-auto rounded-xl border border-gray-800">
              <table className="w-full text-sm">
                <thead className="border-b border-gray-800 bg-gray-900 text-left">
                  <tr>
                    <th className="px-4 py-3 text-gray-400 w-12">#</th>
                    <th className="px-4 py-3 text-gray-400">Producto</th>
                    <th className="px-4 py-3 text-gray-400 text-right">Veces vendido</th>
                    <th className="px-4 py-3 text-gray-400 text-right">% de pedidos</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-gray-800">
                  {topProducts.map(([name, count], i) => (
                    <tr key={i} className="text-white hover:bg-gray-800/50 transition-colors">
                      <td className="px-4 py-3">
                        <span className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-xs font-bold ${i === 0 ? "bg-yellow-500/20 text-yellow-400" : i === 1 ? "bg-gray-400/20 text-gray-300" : i === 2 ? "bg-amber-700/20 text-amber-500" : "bg-gray-800 text-gray-500"}`}>
                          {i + 1}
                        </span>
                      </td>
