    ...
    <div className="mb-3">
      ...
      {multiline ? (
        <textarea id={id} value={value} onChange={e => onChange(e.target.value)}
          ...
          className="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-white outline-none focus:border-green-500 min-h-[80px]" />
      ) : (
        <input id={id} type="text" value={value} onChange={e => onChange(e.target.value)}
          placeholder={placeholder}
          className="w-full rounded-lg border border-gray-700 bg-gray-800 px-3 py-2 text-sm text-white outline-none focus:border-green-500" />
      )}
    </div>
  )
}
    ...
    <div className="flex gap-6">
      <div className="w-48 flex-shrink-0 space-y-1">
        {SECTIONS.map(s => (
          <button key={s.key} onClick={() => setSection(s.key)}
            className={`w-full rounded-lg px-3 py-2 text-left text-sm transition-all ${section === s.key ? "bg-green-600 text-white font-medium" : "text-gray-400 hover:text-white hover:bg-gray-800"}`}>
            {s.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex-1">
        ...
        <div className="rounded-xl border border-gray-800 bg-gray-900 p-6">
          ...
          {section === "faq" && (
            <div>
              <p className="mb-4 text-sm text-gray-400">Preguntas frecuentes — se muestran en /faq</p>
              {(getArray("faq.items").length > 0 ? getArray("faq.items") : defaultContent.faq?.items || []).map((item: any, i: number) => (
                <div key={i} className="mb-4 rounded-lg border border-gray-700 bg-gray-800 p-4">
              ...
              <button onClick={() => addItem("faq.items", { question: "", answer: "" })}
                className="mt-2 rounded-lg border border-dashed border-gray-600 px-4 py-2 text-sm text-gray-400 hover:text-white hover:border-gray-500">
                + Agregar pregunta
              </button>
            </div>
          )}
          ...
          {section === "testimonials" && (
            <div>
              <p className="mb-4 text-sm text-gray-400">Testimonios de clientes — se muestran en la home</p>
          ...
          {section === "stats" && (
            <div>
              <p className="mb-4 text-sm text-gray-400">Estadísticas en la home</p>
              ...
              {(getArray("home.stats.items").length > 0 ? getArray("home.stats.items") : defaultContent.home?.stats?.items || []).map((item: any, i: number) => (
                <div key={i} className="mb-3 flex items-center gap-3 rounded-lg border border-gray-700 bg-gray-800 p-3">
              ...
              <button onClick={() => addItem("home.stats.items", { value: "", label: "" })}
                className="mt-2 rounded-lg border border-dashed border-gray-600 px-4 py-2 text-sm text-gray-400 hover:text-white">
                + Agregar estadística
              </button>
            </div>
          )}
          ...
          {section === "seo" && (
            <div className="space-y-4">
              <p className="text-sm text-gray-400">Meta tags para SEO — aparecen en &lt;title&gt; y &lt;meta description&gt;</p>
          ...
          {section === "features" && (
            <div>
              <p className="mb-4 text-sm text-gray-400">Características / ventajas competitivas</p>
