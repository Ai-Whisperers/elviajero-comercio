      ...
      {showBulk && (
        <div className="mb-6 rounded-xl border border-zinc-800/60 bg-zinc-900/50 p-4 space-y-3">
          ...
          <div className="flex gap-3">
            <select value={bulkCategory} onChange={e => setBulkCategory(e.target.value)} className="rounded bg-zinc-800 px-3 py-2 text-sm text-white border border-zinc-700/60">
              <option value="">Todas las categorías</option>
              {categories.map(c => <option key={c} value={c}>{c}</option>)}
            </select>
            <input type="number" value={bulkPercent} onChange={e => setBulkPercent(parseInt(e.target.value) || 0)} placeholder="%" className="w-24 rounded bg-zinc-800 px-3 py-2 text-sm text-white border border-zinc-700/60" />
            <span className="text-sm text-zinc-400 self-center">% {bulkPercent >= 0 ? "aumento" : "descuento"}</span>
            <button onClick={applyBulk} className="rounded-lg bg-emerald-600 px-4 py-2 text-xs font-semibold text-white hover:bg-emerald-500">Aplicar</button>
          </div>
        </div>
      )}
      ...
      {showNew && (
        <div className="mb-6 rounded-xl border border-zinc-800/60 bg-zinc-900/50 p-4 space-y-3">
          <div className="grid grid-cols-2 gap-3">
            <input value={newForm.name || ""} onChange={e => setNewForm({...newForm, name: e.target.value})} placeholder="Nombre" className="rounded bg-zinc-800 px-2 py-1 text-sm text-white border border-zinc-700/60" />
            <input value={newForm.price || ""} onChange={e => setNewForm({...newForm, price: e.target.value})} placeholder="Precio (Gs.)" className="rounded bg-zinc-800 px-2 py-1 text-sm text-white border border-zinc-700/60" />
            <input value={newForm.category || ""} onChange={e => setNewForm({...newForm, category: e.target.value})} placeholder="Categoría" className="rounded bg-zinc-800 px-2 py-1 text-sm text-white border border-zinc-700/60" />
            <input type="number" value={newForm.stock ?? 0} onChange={e => setNewForm({...newForm, stock: parseInt(e.target.value) || 0})} placeholder="Stock" className="rounded bg-zinc-800 px-2 py-1 text-sm text-white border border-zinc-700/60" />
          </div>
      ...
      <div className="overflow-x-auto max-h-[70vh] rounded-xl border border-zinc-800/60">
        <table className="w-full text-sm">
          <thead className="sticky top-0 z-10 border-b border-zinc-800/60 bg-zinc-900/50 text-left">
            <tr><th className="px-4 py-3 text-zinc-400">Img</th><th className="px-4 py-3 text-zinc-400">Nombre</th><th className="px-4 py-3 text-zinc-400">Precio</th><th className="px-4 py-3 text-zinc-400">Stock</th><th className="px-4 py-3 text-zinc-400">Cat.</th><th className="px-4 py-3 text-zinc-400">Acción</th></tr>
          </thead>
              ...
              <tr key={p.id || i} className="text-white hover:bg-zinc-800/50">
                {editing === i ? (
                  <>
                    ...
                    <td className="px-4 py-2 flex gap-2"><button onClick={save} className="text-xs text-emerald-400 hover:underline">Guardar</button><button onClick={() => setEditing(null)} className="text-xs text-zinc-500 hover:underline">Cancelar</button></td>
                  </>
                ) : (
                  <>
                    <td className="px-4 py-3">
                      {p.image_url ? <img src={p.image_url} alt="" className="h-10 w-10 rounded border border-zinc-700/60 object-cover" /> : <div className="h-10 w-10 rounded border border-zinc-700/60 bg-zinc-800 flex items-center justify-center text-zinc-400 text-xs">—</div>}
                    </td>
                    <td className="px-4 py-3">{p.name}</td>
                    <td className="px-4 py-3">{p.price}</td>
                    <td className="px-4 py-3"><span className={`${(p.stock || 0) > 5 ? "text-emerald-400" : (p.stock || 0) > 0 ? "text-yellow-400" : "text-red-400"}`}>{p.stock ?? 0}</span></td>
